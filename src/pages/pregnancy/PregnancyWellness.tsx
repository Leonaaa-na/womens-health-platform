...
function PregnancyWellness() {
  ...
  useEffect(() => {
    const savedSleep = localStorage.getItem(
      "pregnancySleep"
    );
    ...
    const savedRest = localStorage.getItem(
      "pregnancyRest"
    );

    const savedNote = localStorage.getItem(
      "pregnancyWellnessNote"
    );
    ...
    if (savedExercise) {
      setExercise(savedExercise === "true");
    }

    if (savedRest) {
      setRest(savedRest === "true");
    }

    if (savedNote) {
      setWellnessNote(savedNote);
    }
  }, []);

  const updateSleep = (hours: number) => {
    const newSleep = Math.max(0, Math.min(hours, 12));
      ...
      <main className="mx-auto max-w-md space-y-5 px-5 py-6">
        ...
        <section className="rounded-3xl bg-pink-600 p-6 text-white shadow-lg">
          ...
          <p className="mt-4 text-sm leading-6 opacity-90">
            Small moments of rest, movement, and self-care
            can help you feel supported during your pregnancy.
          </p>
        ...
        <section className="rounded-3xl bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Check off the habits you've completed.
          </p>
        ...
        <section className="rounded-3xl bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Write something about how you're feeling today.
          </p>

          <textarea
            value={wellnessNote}
            onChange={(event) =>
              setWellnessNote(event.target.value)
            }
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-gray-600">
                ...
                of your healthcare professional. If an
                activity or exercise causes discomfort, stop
