...
const activityOptions = ["Walking", "Stretching", "Yoga", "Cycling", "Dancing", "Home workout", "Other"];
const intensityOptions = ["Gentle", "Moderate", "Vigorous"];
...
function WellnessTracker() {
  ...
  // Today's wellness row (water + activities) — one per day on the backend
  useEffect(() => {
    const load = async () => {
      try {
        const response = await apiClient.get(`/trackers/wellness?from=${today}&to=${today}`);
        const row: BackendWellness | undefined = response.data?.data?.[0];
  ...
  // Saves just the fields passed in; the rest of today's row is left alone
  const saveToday = (fields: { waterGlasses?: number; activities?: Activity[] }) =>
    apiClient.post("/trackers/wellness", { date: today, context: "cycle", ...fields });

  const changeWater = async (change: number) => {
    const previous = water;
    const next = Math.max(0, water + change);
    ...
    try {
      ...
      setWater(previous);
      setMessage(errorMessage(error, "Could not update water intake."));
    }
  };
  ...
  const handleAddExercise = async (event: React.FormEvent) => {
    ...
    if (!exerciseType || !duration || !intensity) {
      setMessage("Please complete the exercise information.");
      return;
    }
    ...
    try {
      await saveToday({ activities: next });
      ...
      setMessage(errorMessage(error, "Could not log activity."));
    } finally {
      setSaving(false);
    }
  };
  ...
  const deleteExercise = async (id: string) => {
    ...
    try {
      ...
      setActivities(previous);
      setMessage(errorMessage(error, "Could not delete activity."));
    }
  };

  const totalExerciseMinutes = activities.reduce((total, a) => total + a.duration, 0);
    ...
    <div className="min-h-screen bg-pink-50 pb-24">
      ...
      <header className="bg-white px-5 pb-5 pt-8 shadow-sm">
        <div className="mx-auto flex max-w-md items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Take care of yourself throughout your cycle</p>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-md space-y-5 px-5 py-6">
        ...
        <section className="rounded-3xl bg-green-600 p-6 text-white shadow-lg">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold">Your wellbeing matters</h2>
              <p className="mt-1 text-sm opacity-80">
                Keep track of hydration, movement and healthy daily habits.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-3xl bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-bold text-gray-900">💧 Water intake</h2>
              <p className="mt-1 text-sm text-gray-500">Keep track of the water you drink today.</p>
            </div>
            <span className="text-2xl">💧</span>
          </div>

          <div className="mt-5 rounded-2xl bg-blue-50 p-5 text-center">
            <p className="text-sm text-gray-500">Glasses logged</p>
            <p className="mt-1 text-4xl font-bold text-blue-600">{water}</p>
        ...
        <section className="rounded-3xl bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-bold text-gray-900">🏃 Today's movement</h2>
              <p className="mt-1 text-sm text-gray-500">Activities you've logged today</p>
            </div>
            <span className="text-2xl">🌸</span>
          </div>
          ...
          <div className="mt-5 rounded-2xl bg-green-50 p-5 text-center">
            ...
            <p className="mt-1 text-3xl font-bold text-green-600">{totalExerciseMinutes}</p>
        ...
        {/* Log Exercise */}
        <section className="rounded-3xl bg-white p-5 shadow-sm">
          <h2 className="mb-5 text-lg font-bold text-gray-900">Log an activity</h2>

          <form onSubmit={handleAddExercise} className="space-y-5">
            <div>
              <label className="mb-2 block text-sm font-semibold text-gray-700">Activity type</label>
              <select
                value={exerciseType}
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <span className="absolute right-4 top-3 text-sm text-gray-400">min</span>
              </div>
            </div>

            <div>
              <label className="mb-2 block text-sm font-semibold text-gray-700">Intensity</label>
              <select
                value={intensity}
        ...
        <section className="rounded-3xl bg-white p-5 shadow-sm">
          ...
          {activities.length === 0 ? (
            <div className="mt-5 rounded-2xl bg-gray-50 p-5 text-center">
              ...
              <p className="mt-2 text-sm font-medium text-gray-700">No activities logged today</p>
              <p className="mt-1 text-xs text-gray-500">Add an activity above.</p>
            </div>
          ) : (
            <div className="mt-5 space-y-3">
              {activities.map((activity) => (
                <div key={activity.id} className="rounded-2xl bg-green-50 p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <span className="rounded-full bg-white px-2 py-1 text-xs font-medium text-green-700">
                        {activity.intensity}
                      </span>
                      <h3 className="mt-2 font-bold text-gray-900">{activity.type}</h3>
                      <p className="mt-1 text-xs text-gray-500">{activity.duration} minutes</p>
                      {activity.notes && <p className="mt-2 text-xs text-gray-500">{activity.notes}</p>}
                    </div>
                    <button
                      onClick={() => deleteExercise(activity.id)}
        ...
        <section className="rounded-3xl border border-green-200 bg-green-50 p-5">
          <div className="flex gap-3">
            ...
            <div>
              <h2 className="font-bold text-gray-900">Wellness tip</h2>
              <p className="mt-2 text-sm leading-6 text-gray-600">
                Listen to your body. Gentle movement, enough rest and staying hydrated can
                support your overall wellbeing. Choose activities that feel comfortable for you.
              </p>
            </div>
          </div>
        </section>
      ...
      <nav className="fixed bottom-0 left-0 right-0 border-t border-gray-200 bg-white">
        <div className="mx-auto flex max-w-md items-center justify-around px-3 py-3">
          ...
          <button onClick={() => navigate("/period-tracker/nutrition")} className="flex flex-col items-center gap-1 text-xs text-gray-400">
